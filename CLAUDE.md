...
- **`handsdown_au.h`** — the base alpha layer (`L_HD`), using the "Hands Down Gold" layout with home-row mods (e.g. `HD_A = RSFT_T(KC_A)`). Defines `HD_<pos>` macros (e.g. `HD_LT4`, `HD_LM2`) named by _physical position_ (Left/Right, Top/Middle/Bottom row, column 0-4, plus `LH1/LH2`/`RH1/RH2` for the thumb keys) rather than by legend, so the position grid in `keymap.c`'s `LAYOUT(...)` call stays stable even as the alpha arrangement changes.
